    ...
    async fn invoke(&self, args: Value) -> ToolResult {
        ...
        if matches!(direction.as_str(), "up" | "down") {
            if let (Some(index), Some(wid)) = (element_index, window_id) {
                ...
                let ax_result =
                    cua_driver_core::tool::spawn_blocking_with_authorization(move || -> anyhow::Result<(bool, bool)> {
        ...
        let wheel_target: Option<WheelTarget> = if let Some(element_ptr) = pre_focus_ptr {
            ...
            let wid = window_id;
            let target_task = cua_driver_core::tool::spawn_blocking_with_authorization(move || {
        ...
        if let Some(target) = wheel_target {
            ...
            let result = focus_guard::with_focus_suppressed(
                ...
                || async move {
                    cua_driver_core::tool::spawn_blocking_with_authorization(move || -> anyhow::Result<()> {
        ...
        let result = focus_guard::with_focus_suppressed(
            ...
            || async move {
                ...
                if let Some(element_ptr) = pre_focus_ptr {
                    let _ = cua_driver_core::tool::spawn_blocking_with_authorization(move || {
