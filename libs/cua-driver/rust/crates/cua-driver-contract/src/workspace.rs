...
fn contract<I: ToolInput>(description: &str, read_only: bool, destructive: bool) -> ToolContract {
    ToolContract {
        ...
        description: format!("{description} Workspace workflow: keep one session across conversation turns; create_workspace, then use ordinary list_apps and launch_app with a name or bundle_id when host-enabled. Each launch_app call creates a new isolated instance in the workspace; pass urls to open a project or document. available_apps lists optional legacy launch_workspace_app aliases, not the installed app catalog. Legacy aliases each launch once. Keep the session open for follow-ups; end_session revokes workspace and window grants. Use launched_app.pid/window_id to observe and act. Input stays background; reveal_workspace requires an explicit desktop-switch request. Read background_input.routes before keyboard or pixel input. Missing screenshots can leave usable accessibility data; use include_screenshot:false for AX readback. On a partial launch, inspect workspace_state and move the retained launched_app with move_window_to_workspace before retrying. Never retry by launching another process."),
