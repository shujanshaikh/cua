...
This is supporting development evidence, **not desktop E2E certification**. The final tested code SHA is recorded below after the candidate is committed; any later evidence-only commit is accounted for separately.
...
| Recording | Selected trajectory recording succeeded; display video was refused, no physical-cursor sample file was created, and another session could not read/stop the recording or read its finalized state. |
...
- TypeScript typecheck/build: passed. The initial `npm test` could not install its Electron binary (`fetch failed`); four native-library cases were initially skipped before local staging. Separate staged binding results are recorded with the final candidate below.
